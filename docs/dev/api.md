...
- `user_ids` — список `tg_id`, подключённых к этому inbound; пустой список или отсутствие поля означает "все пользователи ноды" (поведение по умолчанию, как до появления этой модели).
